#!/usr/bin/env python3
"""Runs the zzdds Java waitset example: Subscriber and Publisher as two
separate JVM processes, communicating over real UDP DDS discovery (matching
how the other three bindings' two binaries work). Run build.py first.
...
# waitset's own binaries self-terminate once they've written/received all 10
# samples and torn down (see docs/design/waitset-reference-app.md) -- no
# external signaling needed, just a generous ceiling (comfortably above the
# 25s/30s watchdog deadlines each binary's own main loop uses) in case
# something hangs.
PROC_TIMEOUT_S = 40
