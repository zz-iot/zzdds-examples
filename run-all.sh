...
if [ "$C_BINDING" -eq 1 ] && [ "$CPP_BINDING" -eq 1 ] && [ "$JAVA_BINDING" -eq 1 ]; then
    run_section "interop/hello-world-cross-binding" env ZZDDS_ZIG_OUT="$ZZDDS_ZIG_OUT" \
        "$SCRIPT_DIR/interop/hello-world-cross-binding-smoke-test.sh"
else
    skip_or_fail "interop/hello-world-cross-binding" "needs c-binding, cpp-binding, and java-binding"
fi
